...
def evaluate_scenes(
    gt_scenes, pred_scenes, return_mistakes=False, n_frames_miss_tolerance=2
):
    """
...
def mAP_f1_p_fix_r(one_hot_pred, gt_scenes, fixed_r=0.70654, skip_map_miou=True):
    if fixed_r > 0:
        ...
        while h_thr - l_thr > eps:
            cur_thr = (l_thr + h_thr) / 2.0
            precision = recall = f1 = tp = fp = fn = 0
            for file_name, pred in one_hot_pred.items():
                pred_scenes = predictions_to_scenes(
                    (pred > np.array([cur_thr])).astype(np.uint8)
                )
                _, _, _, (tp_, fp_, fn_) = evaluate_scenes(
                    gt_scenes[file_name], pred_scenes
                )
                tp += tp_
                fp += fp_
                fn += fn_
        ...
        for file_name, pred in one_hot_pred.items():
            pred_scenes = predictions_to_scenes(
                (pred > np.array([l_thr])).astype(np.uint8)
            )
            _, _, _, (tp_, fp_, fn_) = evaluate_scenes(
                gt_scenes[file_name], pred_scenes
            )
    ...
    for file_name, pred in one_hot_pred.items():
        if len(pred) == 0:
            continue
        pred_scenes = predictions_to_scenes(
            (one_hot_pred[file_name] > thresholds[best_idx]).astype(np.uint8)
        )
        y_true_scene, y_pred_scene = evaluate_scenes_mAP(
            gt_scenes[file_name], pred_scenes, pred
        )
